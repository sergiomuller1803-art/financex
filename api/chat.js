export default async function handler(req, res) {
  ...
  if (!apiKey) return res.status(500).json({ error: 'GEMINI_API_KEY não configurada no servidor Vercel.' });
  ...
  try {
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models//v1beta/models/gemini-pro:generateContent?key=${apiKey}`,
      {
        method: 'POST',
